...
/*Strings */
strOrderEnabled = "order-enabled";
strOrderDisabled = "order-disabled";
strFecharPedido = "Fechar pedido";
strHidden = "hidden";
/*add_dish will add a new dish on order object and mark the selected product*/
function addDish(itemId, itemName, itemPrice) {
    ...
    if (markedDish !== "" & markedDrink !== "" & markedDessert !== "") {
        orderReady.classList.add(strOrderEnabled);
        orderReady.classList.remove(strOrderDisabled);
        orderText.innerHTML = strFecharPedido;
    }
}
/*add_drink will add a new drink on order object and mark the selected product*/
function addDrink(itemId, itemName, itemPrice) {
...
function addDessert(itemId, itemName, itemPrice) {
...
function addOrder() {
    ...
    if (markedDish !== "" & markedDrink !== "" & markedDessert !== "") {
        ...
        orderFinalized.classList.remove(strHidden);
        /*dish name */
...
function cancelOrder() {
    let orderFinalized = document.querySelector(".order-confirmed-back");
    orderFinalized.classList.add(strHidden);
}
function endOrder() {
    let whatsapp_message = "Olá, gostaria de fazer o pedido:\n- Prato: " + order.dish[0] + "\n- Bebida: " 
    + order.drink[0] + "\n- Sobremesa: " + order.dessert[0] + "\nTotal: R$ " + order.price.toFixed(2);
    let whatsapp_link = "https://wa.me/1111111111111?text=" + encodeURIComponent(whatsapp_message);
